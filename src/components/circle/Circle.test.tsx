...
describe("Circle Component", () => {
  ...
  it("renders circle with the incremented number as text content", () => {
    // Render the Circle component with isActive prop set to true and the value "3" as its child
    const { getByText } = render(<Circle isActive={true}>3</Circle>);
  ...
  it("renders a default circle without crashing", () => {
    // Render the Circle component with isActive prop set to false and without providing any child value
    const { container } = render(<Circle isActive={false}>0</Circle>);
